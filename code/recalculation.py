        ...
        while True:
            try:
                ...
                for interface in interfaces:
                    ...
                    new_txload = round(
                        self.coefficient * old_txload + 
                        (1 - self.coefficient) * current_txload)

                    if abs(new_txload - old_txload) > self.threshold:
                        interface.set_txload(current_txload)
                        self.router.restart_eigrp()
                        logging.critical('Metric is recalculated for router: {router}'
                            .format(router=self.router.get_host()))
                        logging.critical('Old Load: {old_load}; New Load: {new_load}'
                            .format(old_load=old_txload, new_load=current_txload))
                        break
                    else:
                        print('Load change on {interface_name} is less than threshold'
                            .format(interface_name=name))
                        print('New Load: {new_load}; Old Load: {old_load}'
                            .format(new_load=new_txload, old_load=old_txload))
